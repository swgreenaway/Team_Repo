...
def speed_up(monkeypatch):
    # 1) Kill network calls globally (requests)
    try:
        import requests
        class _DummyResp:
        ...
        def _fake_get(*a, **k): return _DummyResp({})
        def _fake_post(*a, **k): return _DummyResp({})
    ...
    # 3) Short-circuit anything that shells out (if your metrics do)
    import subprocess
    monkeypatch.setattr(subprocess, "run", lambda *a, **k: type("R", (), {"returncode": 0, "stdout": "", "stderr": ""})(), raising=False)

    # 4) If your code uses environment flags to force offline/fast paths, set them here
    import os
